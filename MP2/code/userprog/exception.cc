...
void ExceptionHandler(ExceptionType which)
{
  ...
  case SyscallException:
    ...
    case SC_Open:
      val = kernel->machine->ReadRegister(4);
      DEBUG(dbgSys, "Opening filename: " << &(kernel->machine->mainMemory[val]) << "\n");
      {
        char *filename = &(kernel->machine->mainMemory[val]);
    ...
    case SC_Write:
      ...
      DEBUG(dbgSys, "Writing content: " << &(kernel->machine->mainMemory[val]) << "\n");
      DEBUG(dbgSys, "Writing # of char: " << numChar << "\n");
      DEBUG(dbgSys, "Writing fileID: " << fileID << "\n");
      {
    ...
    case SC_Close:
      ...
      DEBUG(dbgSys, "Closing fileID: " << fileID << "\n");
      status = SysClose(fileID);
      DEBUG(dbgSys, "Close succeed: " << ((status == 1) ? "TRUE" : "FALSE") << "\n");
      kernel->machine->WriteRegister(2, status);
      // Set program counters.
      kernel->machine->WriteRegister(PrevPCReg, kernel->machine->ReadRegister(PCReg));
      kernel->machine->WriteRegister(PCReg, kernel->machine->ReadRegister(PCReg) + 4);
