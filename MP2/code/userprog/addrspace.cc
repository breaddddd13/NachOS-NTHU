...
AddrSpace::~AddrSpace()
{
  for(int i = 0;i < numPages; i++)
    Kernel::usedPhysMem[pageTable[i].physicalPage] = false;
  delete pageTable;
}
...
bool AddrSpace::Load(char *fileName)
{
  ...
  for (int i = 0; i < numPages; i++)
  {
    pageTable[i].virtualPage = i; // for now, virt page # = phys page #
    // pageTable[i].physicalPage = i;
    while (Kernel::usedPhysMem[j] == true) j++;
    Kernel::usedPhysMem[j] = true;
  ...
  if (noffH.code.size > 0)
  {
    ...
    Translate(noffH.code.virtualAddr, &physicalAddr, 0);
    executable->ReadAt(
