...
void Thread::Yield()
{
  ...
  if (nextThread != NULL)
  {
    double exec = (double)kernel->stats->totalTicks - startExec;
    this->setExecTime(exec);
    this->setExecStore(execTime);
...
void Thread::Sleep(bool finishing)
{
  ...
  this->setExecTime(exec);
  this->setExecStore(execTime);
  double predict = execTime + predictTime;
  DEBUG(dbgAging, "[D] Tick " << kernel->stats->totalTicks << ": Thread " << ID << " update approximate burst time, from: " << predictTime << ", add " << execTime << " , to " << (predict / 2.0));
  this->setPredictTime(predict / 2.0);
