...
Kernel::Kernel(int argc, char **argv)
{
  ...
  for (int i = 0; i < NumPhysPages; i++) usedPhysPages[i] = false; 
  for (int i = 1; i < argc; i++)
  {
    ...
    else if (strcmp(argv[i], "-ep") == 0)  // add command line : store the priority in the list
    {
      execfile[++execfileNum] = argv[++i];
      ASSERT(i + 1 < argc);
      // prevent the command without the priority
      char *str = argv[++i];
      ASSERT(str[0] >= '0');
      ASSERT(str[0] <= '9');
      int priority = atoi(str);
      ASSERT(priority < 150);
      ASSERT(priority >= 0);
      priorityList[execfileNum] = priority;
...
void Kernel::ExecAll()
{
  ...
  {
    int a = Exec(execfile[i], priorityList[i]);
  }
...
int Kernel::Exec(char *name, int priority)
{
  ...
  t[threadNum]->setPriority(priority);  // store the priority in thread instance
