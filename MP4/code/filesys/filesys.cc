...
#define NumDirEntries 64 // TODO 10 -> 64??
...
bool FileSystem::Remove(char *name)
{
  ...
  directory->WriteBack(tmp);       // flush to disk
  delete fileHdr;
  delete directory;
  delete freeMap;
  return TRUE;
...
// FileSystem::mkdir
// 	Create directory
//    parse the absolute path by '/'
//  <<<<DONE>>>>
//----------------------------------------------------------------------
void FileSystem::mkdir(char *name)
{
  PersistentBitmap *freeMap = new PersistentBitmap(freeMapFile, NumSectors);
  Directory *dir = new Directory(NumDirEntries);
  OpenFile *tmp = directoryFile;
  dir->FetchFrom(directoryFile);
  // e.g. name = /f1/f2/f3/f4
  // token = f1
  // if token has not found in the current directory => create the directory
  //              found => go to the subdirectory
  char *token = strtok(name, "/");
  while (token != NULL)
  {
  ...
  // find a free sector and add the file to current directory
  // allocate the fileheader then store the fileheader to the free sector
  // write back the directory file to the sector
  // update the upper directory
  int freeSector = freeMap->FindAndSet();
  ASSERT(freeSector >= 0);
  ASSERT(dir->Add(token, freeSector, true));
  FileHeader *fh = new FileHeader;
  ASSERT(fh->Allocate(freeMap, DirectoryFileSize));
  fh->WriteBack(freeSector);
  OpenFile *subDirHdr = new OpenFile(freeSector);
  Directory *subDir = new Directory(NumDirEntries);
  subDir->WriteBack(subDirHdr);
  dir->WriteBack(tmp);
  freeMap->WriteBack(freeMapFile);
...
//----------------------------------------------------------------------
void FileSystem::RecursiveRemove(char *name)
{
  PersistentBitmap *freeMap = new PersistentBitmap(freeMapFile, NumSectors);
  DEBUG(dbgFile, "Recursive remove : " << name);
  Directory *directory = new Directory(NumDirEntries);
  Directory *subDir = new Directory(NumDirEntries);
  directory->FetchFrom(directoryFile);
  OpenFile *tmp = directoryFile;
  ...
  else
  {
    ...
    subDir->WriteBack(subFile);
  }
  directory->Remove(prev);
  directory->WriteBack(tmp);
  freeMap->WriteBack(freeMapFile);
  delete freeMap;
  delete subFile;
